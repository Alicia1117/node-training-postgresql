...
const requestListener = async (req, res) => {
    ...
    if (req.url === '/api/credit-package' && req.method === 'GET') {
        try {
            const packages = await AppDataSource.getRepository(
                'CreditPackage'
        ...
        req.on('end', async () => {
            try {
                ...
                const packageSaved = await creditPackageRepo.save(newPackage);
                res.writeHead(200, headers);
                res.write(
                    JSON.stringify({
                        status: 'success',
        ...
        try {
            ...
            const packageDeleted = await AppDataSource.getRepository(
                'CreditPackage'
            ).delete(packageId);
            if (packageDeleted.affected === 0) {
                res.writeHead(400, headers);
                res.write(
                    JSON.stringify({
                        status: 'failed',
            ...
            res.write(
                JSON.stringify({
                    status: 'package successfully deleted',
                })
            );
            res.end();
        } catch (error) {
        ...
        req.on('end', async () => {
            try {
                ...
                const skillSaved = await skillRepo.save(newSkill);
                res.writeHead(200, headers);
                res.write(
                    JSON.stringify({
                        status: 'success',
        ...
        try {
            ...
            const skillDeleted = await AppDataSource.getRepository(
                'Skill'
            ).delete(skillId);
            ...
            res.write(
                JSON.stringify({
                    status: 'skill successfully deleted',
                })
            );
            res.end();
        } catch (error) {
